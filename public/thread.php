...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["thread"])) {
        ...
        if($row = mysqli_fetch_assoc($results)) {
            ...
            echo "<div>" . $row["category"] . "<div style='float:right'>" . $row['creationDate'] . "</div></div>";
        } else {
       ...
       // $sql = "SELECT * FROM comments WHERE threadId='" . $threadId . "' ORDER BY date ASC";
        ...
        while ($row = mysqli_fetch_assoc($results)) {
            ...
            if ($threadContent == true) {
                ...
                echo '<br><a href="profile.php?id=' . $row["userId"] . '"><img class="usericon" width="50" height="50" style="z-index:1" src="script/images/' . $row["userId"] . '.jpg"></a>
                <div style="position:relative; top:-1.3em">' . $row["displayName"] .'</div><div style="position:relative; top:-1.3em; left:4em"><p>' . $row["content"] . '</p></div>';
                echo "<br>";
                $threadContent = false;
